...
export const getAllSavedPosts = asyncHandler(async (req, res) => {
  const user = await User.findById(req.userId)
    .select('savedPosts')
    .populate({
      path: 'savedPosts',
...
export const savePost = asyncHandler(async (req, res) => {
  ...
  return res.json({ message: 'Saved Post Successfully' });
});

export const deleteSavedPost = asyncHandler(async (req, res) => {
  const { postId } = req.params;
  const post = await Post.findById(postId);
  ...
  return res.json({ message: 'Saved Post Deleted Successfully' });
});

export const deleteAllSavedPosts = asyncHandler(async (req, res) => {
  await User.findByIdAndUpdate(req.userId, {
    savedPosts: [],
  });
  return res.json({ message: 'All Saved Posts Cleared!' });
});
